...
class Obstacles extends GameComponents {
  ...
  createMushroomsArray() {
    this.mushrooms = [];
    for (let i = 0; i < 50; i++) {
      let x = (Math.floor(Math.random() * 10) * canvas.width / 10) + this.radius + 25;
      let y = (Math.floor(Math.random() * 10) * canvas.height / 10) + this.radius + 20;
 ...
 hatchSpider() {
    ...
    let startingPosition = this.spiderStartingPositions[Math.floor((Math.random() * 31))];
